...
#define _DUTY_MAX 1800  // 서보의 최대 각도값

// Servo speed control
#define _SERVO_ANGLE 30  // 서보 각도 설정
#define _SERVO_SPEED 150  // 서보의 속도 설정
...
// PID parameters
#define _KP 1.3  // 비례 제어 값
#define _KD 30.0  // 미분 제어 값

#define A 75.45  // 실제 거리가 100일 때 센서가 읽는 값
#define B 363.74  // 실제 거리가 400일 때 센서가 읽는 값
...
float calcDistance(short value) {
  ...
  float val = ((6762.0/(volt-9.0))-4.0) * 10.0;  // cm -> mm 변환
  return 100 + 300.0 / (B - A) * (val - A);  // 실제 거리 반환
  //return val;
}
...
void loop(){
  ...
  if(event_serial){
    ...
    Serial.print(",pterm:");
    Serial.print(map(pterm,-1000,1000,510,610));  // pterm
    Serial.print(",dterm:");
    Serial.print(map(dterm,-1000,1000,510,610));  // dterm
    Serial.print(",duty_target:");
    Serial.print(map(duty_target,1000,2000,410,510));  // duty_target
    Serial.print(",duty_curr:");
    Serial.print(map(duty_curr,1000,2000,410,510));  // duty_curr
    Serial.println(",Min:100,Low:200,dist_target:275,High:310,Max:410");
  }
}
